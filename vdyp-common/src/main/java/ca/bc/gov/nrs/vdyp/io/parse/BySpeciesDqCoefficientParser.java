 ...
 * </ol>
 * If index is zero or one, the ith coefficient is the value used for Species identified by the
 * number i (determined by calling <code>GenusDefinitionParser.getSpeciesAliases</code> as usual.)
 * If index is two, the first (and only) coefficient given is used as the value for all Species.
 * <p>
 * For example, the values for Species with number s are (row w/index 0 coefficient s, row w/index 1 coefficient s,
 * row w/index 2 coefficient 0).
 * <p>
