 ...
 * </ol>
 * The result of the parse is a {@link MatrixMap3} of four coefficients indexed by one of the listed diameter
 * values, Species, and then BEC Zone. The BEC Scope field is converted in a (set of) BEC Zones in this way:
 * <ul>
 ...
 * </ul>
 * The diameter value indicates a range from this value (inclusive) to the next listed value (exclusive), 
 * except in the case of the last value, where the upper bound of the range is +infinity.
 * <p>
 * Multiple lines with the same indices are legal, with the last entry winning. Lines starting with 
 * "    " (four blanks) are skipped, regardless of the remainder of the line. There is no parsing
 * termination line format - all lines are always read.
 * <p>
