...
public class PolygonIdentifier {
	...
	public PolygonIdentifier(String base, int year) throws IllegalArgumentException {
		if(base.length()>BASE_LENGTH) throw new IllegalArgumentException("Polygon identifier base \""+base+"\" is too long.");
		if(year<1) throw new IllegalArgumentException("Polygon identifier year "+year+" must be positive.");
		this.base = base;
		this.year = year;
	}

	public static PolygonIdentifier split(String polygonIdentifier) throws IllegalArgumentException {
		if(polygonIdentifier.length()!=ID_LENGTH) throw new IllegalArgumentException("Polygon identifier \""+polygonIdentifier+"\" must be exactly "+ID_LENGTH+" characters.");
