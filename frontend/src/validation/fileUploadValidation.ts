...
export class FileUploadValidation extends ValidationBase {
  ...
  async validateLayerFile(file: File): Promise<boolean> {
    ...
    return await this.validateCSVHeaders(file, expectedHeaders)
  }

  async validatePolygonFile(file: File): Promise<boolean> {
    const expectedHeaders = [
      'FEATURE_ID',
      'MAP_ID',
      'POLYGON_NUMBER',
