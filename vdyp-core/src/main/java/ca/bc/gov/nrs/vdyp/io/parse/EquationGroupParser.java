...
public class EquationGroupParser implements ResourceParser<Map<String, Map<String, Integer>>>{
	...
	public Map<String, Map<String, Integer>> parse(InputStream is, Map<String, Object> control) throws IOException, ResourceParseException {
		...
		// Validate that the cartesian product of SP0 and BEC values has been provided, excluding unused BECs.
		// The original fortran did a check that the number of values read matched a hard coded number.
		...
		sp0Diff.getMissing().stream().map(sp0Key->String.format("Expected mappings for SP0 %s but it was missing", sp0Key)).collect(Collectors.toCollection(()->errors));
		sp0Diff.getUnexpected().stream().map(sp0Key->String.format("Unexpected mapping for SP0 %s", sp0Key)).collect(Collectors.toCollection(()->errors));
		
		for(var entry : result.entrySet()) {
			var becDiff = ExpectationDifference.difference(entry.getValue().keySet(), becKeys);
			var sp0Key = entry.getKey(); 
			becDiff.getMissing().stream().map(becKey->String.format("Expected mappings for BEC %s but it was missing for SP0 %s", becKey, sp0Key)).collect(Collectors.toCollection(()->errors));
			becDiff.getUnexpected().stream().map(becKey->String.format("Unexpected mapping for BEC %s under SP0 %s", becKey, sp0Key)).collect(Collectors.toCollection(()->errors));
		}
