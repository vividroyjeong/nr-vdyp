...
public class FipControlParser {
	...
	ControlFileParser controlParser = new ControlFileParser()
			...
			.record(197, MINIMA, ValueParser.list(ValueParser.FLOAT)) // Minimum Height, Minimum BA, Min BA fully stocked.
		...
		try (var is = klazz.getResourceAsStream(resourceName)) {
			...
			return parse(is, new FileResolver() {
				...
				public InputStream resolve(String filename) throws IOException {
					...
					if(resourceAsStream==null) throw new IOException("Could not load "+filename);
	...
	Map<String, ?> parse(InputStream is, FileResolver fileResolver) throws IOException, ResourceParseException {
		...
		if(jprogram==1) {
			...
			// TODO minima?
			/*
			 * READ(CNTRV(197), 197, ERR= 912 ) FMINH, FMINBA, FMINBAF,FMINVetH 
			 * IF (FMINVetH .le. 0.0) FMINVetH=10.0
			 */
		}
		...
		if(jprogram==1) {
			...
			// TODO
			/*
				READ(CNTRV(197), 197, ERR= 912 ) VMINH, VMINBA, VMINBAeqn,VMINvetH
				IF (VMINVetH .le. 0.0) VMINVetH=10.0
			*/
...
141   FORMAT(25I2)
      DO 140 I=1,25
        if ((NDEBUG(I) .ne.0) .and. (IU_WRITE .gt. 0))
     &         WRITE(IU_WRITE,142) I, NDEBUG(I)
142      FORMAT(' CTR Line 199  has NDEBUG(',i2,') =',I3)
140   CONTINUE

      if (isDBG) then
         call dbgMsg( logr, 0 ) "Debug Switches (Ctl Line 199)" )
         call dbgI4(  logr, 6 ) " 1: Applying BA Limits               ", NDEBUG( 1) )
         call dbgMsg( logr, 6 ) "       0 - Apply BA limits from SEQ043." )
         call dbgMsg( logr, 6 ) "       1 - Do not apply BA limits from SEQ043." )
         call dbgI4(  logr, 6 ) " 2: Applying BA Limits               ", NDEBUG( 2) )
         call dbgMsg( logr, 6 ) "       0 - Apply DQ limits from SEQ043." )
         call dbgMsg( logr, 6 ) "       1 - Do not apply DQ limits from SEQ043." )
         call dbgI4(  logr, 6 ) " 3: Not used                         ", NDEBUG( 3) )
         call dbgI4(  logr, 6 ) " 4: Not implemented. Upper bounds    ", NDEBUG( 4) )
         call dbgMsg( logr, 6 ) "       0 - Will default to (2)." )
         call dbgMsg( logr, 6 ) "       1 - Limits from Ctl Line 108 (GRPBA1)." )
         call dbgMsg( logr, 6 ) "       2 - Limits from Ctl Line 043 (Coast/Interior)" )
         call dbgI4(  logr, 6 ) " 5: Not used                         ", NDEBUG( 5) )
         call dbgI4(  logr, 6 ) " 6: Not used                         ", NDEBUG( 6) )
         call dbgI4(  logr, 6 ) " 7: Not used                         ", NDEBUG( 7) )
         call dbgI4(  logr, 6 ) " 8: Not used                         ", NDEBUG( 8) )
         call dbgI4(  logr, 6 ) " 9: Error handling                   ", NDEBUG( 9) )
         call dbgMsg( logr, 6 ) "       0 - Stop on all errors." )
         call dbgMsg( logr, 6 ) "       1 - Continue to next poly (non-I/O errs)." )
         call dbgI4(  logr, 6 ) "10: Not used                         ", NDEBUG(10) )
         call dbgI4(  logr, 6 ) "11: Not used                         ", NDEBUG(11) )
         call dbgI4(  logr, 6 ) "12: Not used                         ", NDEBUG(12) )
         call dbgI4(  logr, 6 ) "13: Not used                         ", NDEBUG(13) )
         call dbgI4(  logr, 6 ) "14: Not used                         ", NDEBUG(14) )
         call dbgI4(  logr, 6 ) "15: Not used                         ", NDEBUG(15) )
         call dbgI4(  logr, 6 ) "16: Not used                         ", NDEBUG(16) )
         call dbgI4(  logr, 6 ) "17: Not used                         ", NDEBUG(17) )
         call dbgI4(  logr, 6 ) "18: Not used                         ", NDEBUG(18) )
         call dbgI4(  logr, 6 ) "19: Not used                         ", NDEBUG(19) )
         call dbgI4(  logr, 6 ) "20: Not used                         ", NDEBUG(20) )
         call dbgI4(  logr, 6 ) "21: Not used                         ", NDEBUG(21) )
         call dbgI4(  logr, 6 ) "22: Determining primary species      ", NDEBUG(22) )
         call dbgMsg( logr, 6 ) "       0 - Normal " )
         call dbgMsg( logr, 6 ) "       1 - Extra preference for preferred sp. as primary" )
         call dbgI4(  logr, 6 ) "23: Not used                         ", NDEBUG(23) )
         call dbgI4(  logr, 6 ) "24: Not used                         ", NDEBUG(24) )
         call dbgI4(  logr, 6 ) "25: Not used                         ", NDEBUG(25) )
      end if
		 */
	...
	/** 
	 * Loads the information that was in the global arrays STKV, FACTV, and NPCTAREA in Fortran
	 */
