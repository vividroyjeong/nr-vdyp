...
public enum CFSTreeSpecies implements SI32Enum<CFSTreeSpecies> {
	...
	private static java.util.HashMap<Integer, CFSTreeSpecies> mappings;

	private static java.util.HashMap<Integer, CFSTreeSpecies> getMappings() {
		if (mappings == null) {
			synchronized (CFSTreeSpecies.class) {
				if (mappings == null) {
					mappings = new java.util.HashMap<Integer, CFSTreeSpecies>();
				}
			}
		}
		return mappings;
	...
	private CFSTreeSpecies(int value, int cfsSpeciesNumber) {
		intValue = value;
	...
	public static CFSTreeSpecies forValue(int value) {
