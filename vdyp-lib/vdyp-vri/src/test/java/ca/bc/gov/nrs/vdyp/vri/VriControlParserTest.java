...
class VriControlParserTest {
	...
	void testParseE061() throws Exception {
		...
		assertThat(
				result, (Matcher) controlMapHasEntry(
						...
						mmHasEntry(instanceOf(ComponentSizeLimits.class)
								, "AC", Region.COASTAL
						)
				)
		);

		var mm = Utils.<MatrixMap2<String, Region, ComponentSizeLimits>>expectParsedControl(result, ControlKey.SPECIES_COMPONENT_SIZE_LIMIT, MatrixMap2.class);
	...
	void testParseMinima() throws Exception {
		...
		assertThat(
				result, (Matcher) controlMapHasEntry(
						...
						// Includes modifiers from 198
						allOf(
								hasEntry(is(BaseControlParser.MINIMUM_HEIGHT), is(6.0f)), hasEntry(
										is(BaseControlParser.MINIMUM_BASE_AREA), is(0.0f)
								), hasEntry(
										is(BaseControlParser.MINIMUM_PREDICTED_BASE_AREA), is(2.0f)
	...
	void testParseV7R_RIP1() throws Exception {
		...
		assertThat(
				result, (Matcher) controlMapHasEntry(
						ControlKey.VRI_INPUT_YIELD_POLY, instanceOf(StreamingParserFactory.class)
				)
		);
	}
	...
	void testParseV7R_RIL1() throws Exception {
		...
		assertThat(
				result, (Matcher) controlMapHasEntry(
						ControlKey.VRI_INPUT_YIELD_LAYER, instanceOf(StreamingParserFactory.class)
				)
		);
	}
	...
	static Map<String, ?> parseWithAppendix(BaseControlParser parser, String... lines)
		...
		try (
				InputStream baseIs = TestUtils.class.getResourceAsStream(CONTROL_FILE);
