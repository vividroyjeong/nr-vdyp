...
class UtilizationOperationsTest {
	...
	void testUtilizationOperations() throws Exception {
		...
		var polygon = VdypPolygon.build(pb -> {
			...
			pb.addLayer(VdypLayer.build(lb -> {
				...
				lb.closeUtilizationVolumeNetOfDecayAndWasteByUtilization(0.0000f, 0.3788f, 6.8324f, 19.8375f, 550.5741f);
				lb.closeUtilizationVolumeNetOfDecayWasteAndBreakageByUtilization(0.0000f, 0.3623f, 6.5384f, 18.9555f, 523.1597f);
				...
				lb.addSpecies(VdypSpecies.build(sb -> {
					...
					sb.closeUtilizationVolumeNetOfDecayWasteAndBreakageByUtilization(0.0000f, 0.0565f, 0.5005f, 5.5948f, 429.1300f);
				...
				lb.addSpecies(VdypSpecies.build(sb -> {
					...
					sb.baseArea(0.00000f, 0.00502f, 0.01063f /* <-- too small; this will be replaced by the minimum */, 0.02284f, 0.36143f);
					...
					sb.quadMeanDiameter(6.1f, 31.5f, 7.4999f /* <-- too small; this will be replaced by the minimum */, 13.7f, 18.2f, 42.1f);
				}));
			}));
		});
	...
	private void testScaling(VdypUtilizationHolder post, VdypUtilizationHolder pre, float scalingFactor) {
		...
		testScalingWasPerformed(post.getCloseUtilizationVolumeByUtilization(), pre.getCloseUtilizationVolumeByUtilization(), scalingFactor);
		testScalingWasPerformed(post.getCloseUtilizationVolumeNetOfDecayAndWasteByUtilization(), pre.getCloseUtilizationVolumeNetOfDecayAndWasteByUtilization(), scalingFactor);
		testScalingWasPerformed(post.getCloseUtilizationVolumeNetOfDecayByUtilization(), pre.getCloseUtilizationVolumeNetOfDecayByUtilization(), scalingFactor);
		testScalingWasPerformed(post.getCloseUtilizationVolumeNetOfDecayWasteAndBreakageByUtilization(), pre.getCloseUtilizationVolumeNetOfDecayWasteAndBreakageByUtilization(), scalingFactor);
		testScalingWasNotPerformed(post.getLoreyHeightByUtilization(), pre.getLoreyHeightByUtilization(), scalingFactor);
		testScalingWasNotPerformed(post.getQuadraticMeanDiameterByUtilization(), pre.getQuadraticMeanDiameterByUtilization(), scalingFactor);
		testScalingWasPerformed(post.getTreesPerHectareByUtilization(), pre.getTreesPerHectareByUtilization(), scalingFactor);
		testScalingWasPerformed(post.getWholeStemVolumeByUtilization(), pre.getWholeStemVolumeByUtilization(), scalingFactor);
	}
